...
const saveBooksToFile = async () => {
    try {
        await fs.promises.writeFile("./Books.json", JSON.stringify(books, null, 2), "utf8");
    } catch (error) {
...
const server = http.createServer(async (request, response) => {
    ...
    switch (true) {
        ...
        case (request.method === "PUT" || request.method === "PATCH") && pathname.startsWith("/books/"):
            try {
...
const startServer = async () => {
    await initializeBooksFile(); // Ensure Books.json is initialized
    server.listen(PORT, () => {
